...
exports.loans = async () => {
  ...
  if (!loans) {
    ...
    fs.writeFileSync('loans.json', JSON.stringify(loans))
    for (loan of loans) {
...
exports.holds = async () => {
  const res = await fetchPage('holds')
  const holds = h.getHolds(res.data)

  if (!holds) {
    console.log('You do not have any holds ready for pickup.')
  } else {
    for (hold of holds) {
      console.log(`${hold.title} is now ready for pickup!`)
    }
  }
}
